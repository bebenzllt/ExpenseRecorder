...
def Save(event=None):
	...
	#จะใส่ csv โดยมีรหัส time stamp
	try:
		total = int(price)*int(quantity)
		# total คือ รวมค่าใช้จ่ายทั้งหมด

		#ทำการบันทึกเวลาตอนตอนเซฟ
		stamp = datetime.now() # current date and time
		date_time = stamp.strftime("%m/%d/%Y %H:%M:%S")
		today = stamp.strftime('%a')
		#สร้างรหัส id
		...
		with open('savedata1.csv','a',encoding='utf-8',newline='') as f:
			...
			data = [transactionid,expense,price,quantity,total,d_time] #ข้อมูลที่ต้องการใส่
...
def read_csv():
	with open('savedata1.csv',newline='',encoding='utf-8') as f:
		fr = csv.reader(f)
...
header = ['รหัสรายการ','รายการ','ค่าใช้จ่าย','จำนวน','รวม','วัน-เวลา']
...
headerwidth = [120,80,80,80,80,170] #หน่วยpixel
for h,w in zip(header,headerwidth):
	resulttable.column(h,width=w)

#เขียนโปรแกรมให้มีความสามารถในการค้นหาข้อมูลที่อยู่ในไฟล์ csv
#ใช้หัวข้อ dictionary ให้เก็บข้อมูลลักษณะแบบนี้เพื่อให้ค้นหาข้อมูลได้ง่าย data = {'202201121628304937':['202201121628304937','แกงส้ม',30,2,60,'พุธ-01/12/2022 16:28:54']}

#สร้างตัวแปรขึ้นมาเป็น dictionary {}
alltransaction = {}

#ทำการเซฟเป็นไฟล์ csv ซ้ำทับไฟล์เดิม
#แล้วทำการupdate ตารางใหม่อีกรอบ
#ทำการสร้างฟังก์ชัน Update csv

def UpdateCSV():
	with open('savedata1.csv','w',newline='',encoding='utf-8') as f: #การ write csv ต้องใส่ 'w' เป็นการเขียนทับสิ่งที่มีอยู่ในไฟล์เดิม
		fw = csv.writer(f)
		#เตรียมข้อมูลจาก alltransaction ให้เป็น list เพราะฟังก็ชันที่จะเรียกใช้ต้องเป็น list เท่านั้น
		data = list(alltransaction.values())
		#เป็นการ writerow แบบหลายบรรทัด เป็น list ซ้อน list คือเป็นการเขียนแบบ multiple line from nested list [[],[],[]]
		fw.writerows(data)
		print('Table was updated')


#สร้างปุ่ม Delete
#ทำการคลิกรายการที่จะ delete แล้วกดปุ่ม delete

#สร้าง function ตัว delete ข้อมูลขึ้นม
def DeleteRecord(event = None):
	#สร้าง massagebox เพื่อถาม yes no ว่าจะลบจริงไหม
	check = messagebox.askyesno('Confirm?','คุณต้องการที่จะลบรายการค่าใช้จ่ายนี้ใช่หรือไม่')
	print('YES/NO',check)

	if check == True:
		print('delete')
		select = resulttable.selection() #ดูรหัสเฉพาะตัวที่เลือก
		#print(select)
		data = resulttable.item(select) #ดึงข้อมูลตัวรหัสที่เลือก เป็นข้อมูล dictionary
		data = data['values'] #ดึงข้อมูลที่เป็น dictionary
		transactionid = data[0] #ตัวที่เรียกมาจาก table เป็น integer แต่ในdictionary เป็นข้อความ(string)
		#print(transactionid)
		#print(type(transactionid))  #ดังนั้นต้องแปลงเป็นstringด้วย
		del alltransaction[str(transactionid)] #เป็นการลบข้อมูลที่อยู่ใน dictionary
		#print(alltransaction)
		UpdateCSV() #ทำการupdate csv file
		update_table() #ทำการเรียกฟังก์ชัน update table ให้เราอัตโนมัติ
	else:
		print('cancel')
BDelete = ttk.Button(T2,text='delete',command=DeleteRecord)
BDelete.place(x=45,y=540)

#ถ้ามีการใช้ bind จะต้องใส่ตรงฟังก์ชันว่า event = None
#เมื่อกดปุ่ม Delete ก็สามารถลบข้อมูลได้
...
def update_table():
	...
	#แก้ error ครั้งแรก ดักจับ error กรณีไม่มี File แค่โปรแกรมยังคง run ได้
	#ครั้งต่อมาจะไม่มีปัญหาแล้ว
	try:	
	#ดึง data มาก่อน
		data = read_csv() #เป็น list ซ้อน list
		for d in data:
			#creat tranaction data
			#เอา data มาใส่ใน alltransaction
			alltransaction[d[0]] = d # d[0] = trtranactionid
			resulttable.insert('','0',value=d)
	        #เมื่อมีการกด save จะต้องมีการอัพเดตไฟล์ใหม่ด้วย
		print(alltransaction)	       
	except:
		print('No File') #ให้ผ่านไปเลยไม่มีการ insert ข้อมุูล
